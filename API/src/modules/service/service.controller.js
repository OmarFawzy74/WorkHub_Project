...
import categories_model from "../../../DB/models/category_model.js";
import fs from "fs";
...
export const getAllServices = async (req, res, next) => {
  try {
    ...
    const modifiedServices = services.map((service) => {
      ...
      modifiedService.freelancerId.image_url =
        "http://" +
        req.hostname +
        ":3000/" +
        modifiedService.freelancerId.image_url;
      modifiedService.serviceCover_url =
        "http://" + req.hostname + ":3000/" + modifiedService.serviceCover_url;
      modifiedService.serviceImages_url = modifiedService.serviceImages_url.map(
        (image_url) => {
    ...
    res
      .status(200)
      .json({ success: true, message: "here u r", services: modifiedServices });
  } catch (error) {
    console.log(error);
...
export const searchforService = async (req, res, next) => {
  try {
        const query = req.params.query;
        const services = await Service.find({
        $or: [
            { serviceTitle: new RegExp(query, "i") },
            { serviceDesc: new RegExp(query, "i") },
            { serviceShortTitle: new RegExp(query, "i") },
            { serviceShortDesc: new RegExp(query, "i") },
        ],
        }).populate("freelancerId").populate("serviceCategoryId").populate("reviews").populate("orders");

        if (!services[0]) {
            res.status(404).json({ success: false, msg: "No Results Found" });
        }
...
export const getServicesByCategoryId = async (req, res, next) => {
  try {
    ...
    const services = await Service.find({ serviceCategoryId: categoryId })
      .populate("freelancerId")
      .populate("serviceCategoryId")
      .populate("reviews")
      .populate("orders");
  ...
  try {
    ...
    const update = {
      $set: {
        ...
        serviceShortTitle: serviceShortTitle,
...
export const getServiceById = async (req, res, next) => {
  try {
    ...
    service.freelancerId.image_url =
      "http://" + req.hostname + ":3000/" + service.freelancerId.image_url;
    ...
    service.serviceCover_url =
      "http://" + req.hostname + ":3000/" + service.serviceCover_url;
...
export const getFreelancerServices = async (req, res, next) => {
  try {
    ...
    const services = await Service.find({ freelancerId: id })
      .populate("freelancerId")
      .populate("serviceCategoryId");
...
export const updateServiceImages = async (req, res, next) => {
  try {
    if (!req.files) {
      return res
        .status(404)
        .send({ success: false, message: "images are required" });
    }
