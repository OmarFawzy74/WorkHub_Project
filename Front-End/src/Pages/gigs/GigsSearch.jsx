...
function GigsSearch() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:3000/api/services/searchforService/" + query)
      .then((resp) => {
  ...
  return (
    <div className={pathname !== "/adminGigs" ? "gigs" : "adminGigs"}>
      <div className={sidebarStatus() ? "gigsContainer" : "allGigsContainer"}>
        ...
        <div className="title">
          <h1><Link reloadDocument className="breadcrumbsLink" to={"/gigs"}><img className="homeIconImg" src="../img/marketplace-1.png" /> All Services {'>'}</Link>{query}</h1>
        </div>
